...
class HrFrequencyDDS(ConductorParameter):
    autostart = True
    priority = 2
    ...
    def initialize(self,config):
        ...
        initial_request_low =  {'top_ad9956_0': {'frequency': self.dark_frequency, 'output':'low'} }
        self.cxn.rf.dicfrequencies(json.dumps(initial_request_low))
        initial_request_high =  {'top_ad9956_0': {'frequency': self.dark_frequency+self.dark_offset, 'output':'high'} }
    ...
    def update(self):
        if self.value is not None:
            print('clock_aom.hr_demod_frequency', self.value)
